...
fn stats_from_event(event_file: &Path) {
	println!("{}{:?}", indent(1), event_file);
	let rdr = csv::Reader::from_file(event_file).unwrap();
	let mut rdr = rdr.has_headers(false).delimiter(b',').flexible(true);
	for row in rdr.records() {
		let row = row.unwrap();
		println!("{}{:?}", indent(2), row);
	}
}
