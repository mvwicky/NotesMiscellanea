...
class GenReadme(object):
	def __init__(self, config):
		...
		self.contents = FolderContents(self.cfg['to_root']).contents
	...
	def open_html(self):
		res_names = self.cfg['resource_names']
	...
	def open_page(self):
		...
		page_header = self.cfg['page_header']
		...
		desc = self.cfg['page_description']
	...
	def content(self):
		headings = self.cfg['sub_headings']
		for heading in headings.keys():
			...
			for sub in headings[heading]:
				...
				a_right = self.html.gen_tag(span, _class='arrow arrow_right', 
											new_line=False, close=True, as_string=True)
				...
				for elem in self.contents[sub_id]:
					...
					path_to_elem = '{}/{}'.format(self.cfg['to_root'], elem)
					if os.path.isdir(path_to_elem):
						elem_cts = [i for i in os.listdir(path_to_elem) if not i.startswith('.')]
						self.html.list_open(_class='sublist', _id=elem_id)
						for e in elem_cts:
							li = self.html.gen_tag(span, text=e, close=True,
												   new_line=False, as_string=True)
							self.html.list_element(text=li)
