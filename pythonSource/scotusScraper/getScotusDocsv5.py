			...
			with open(arg[1], 'wb') as f:
				...
				if totalLength is None:
					...
					for data in res.iter_content():
						...
						self.emit(SIGNAL('update(int)'), int(done))
...
class window(QMainWindow):
	def __init__(self, parent=None):
		...
		self.connect(self.thread, SIGNAL('update(int)'), self.updateStatus)
	...
	def initUI(self):
		...
		for year in years:
			...
			grid.addWidget(yearButton, grid.rowCount(), 0, 1, 1)
