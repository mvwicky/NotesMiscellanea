...
class worker(QThread):
	...
	def run(self):
		with open(self.arg[1], 'wb') as f:
			self.sendMessage("Downloading {}".format(self.arg[0]))
			self.emit(SIGNAL('output(arg)'), self.arg)
			res = requests.get(self.arg[0], stream=True)
...
class window(QWidget):
	def __init__(self, parent=None):
		...
		self.connect(self.thread, SIGNAL('output(arg)'), self.sendMessage)
	...
	def getArgumentAudio(self, year):
		...
		self.threadedDownload(audioPair)
	def getSlipOpinions(self, year):
		...
		self.threadedDownload(slipPair)

	def threadedDownload(self, pairs):
		threads = []
		for i in range(2):
		...
		for th in threads:
			th.join()
		self.sendMessage('Downloaded {} files'.format(len(pairs)))
	def worker(self):
		while True:
			arg = self.queue.get()
			if arg is None:
