...
class window(QMainWindow):
	def __init__(self, parent=None):
		...
		self.transBase = 'http://supremecourt.gov/oral_arguments/'
	...
	def getArgumentTranscripts(self, year):
		...
		yearDir = self.saveDir + year + '\\'
		transDir = yearDir + 'Argument Transcripts\\'
		if not os.path.exists(transDir):
			self.sendMessage('No directory for: {}'.format(transDir))
			try:
				os.makedirs(transDir)
			except:
				self.sendMessage('Problem creating: {}'.format(transDir))
				return 0 
			else:
				self.sendMessage('Created: {}'.format(transDir))
		transURL = '{}argument_transcript/{}'.format(self.transBase, year)
		transSoup = BeautifulSoup(request.urlopen(transURL), 'lxml')
		trans = { 'Links'     : [],
			      'Dockets'   : [],
			      'Filenames' : [],
			      'Pairs'     : [] }
		for cell in transSoup.find_all('td'):
			for a in cell.find_all('a'):
				if 'argument_transcripts' in a.get('href'):
					...
					fileName = '{}{}{}'.format(transDir, self.removeNotAllowedChars(docket), '.pdf')
					trans['Filenames'].append(fileName)
					trans['Pairs'].append((link, fileName))
		self.thread.download(trans['Pairs'])
