...
class FG_Parser(object):
    ...
    def get_ids_csv(self, pit=True, bat=True):
        ...
        if not (bat and pit):
            ...
            ids = dict()
            self.log('Populating {} ID#s from: {}'
                     .format('pitcher' if pit else 'batter', file_name))
    ...
    def get_ids_web(self, pit=True, bat=True, clean=False):
        ...
        if pit and self.pitcher_csv in os.listdir() and not clean:
            self.log('Pitcher ID#s already found in: {}'
                     .format(self.pitcher_csv))
            self.get_ids_csv(True, False)
            return
        if bat and self.batter_csv in os.listdir() and not clean:
            self.log('Batter ID#s already found in: {}'
                     .format(self.batter_csv))
            self.get_ids_csv(False, True)
            return
        ...
        if not (bat and pit):
            ...
            self.log('Populating {} ID#s'
                     .format('pitcher' if pit else 'batter'))
            ...
            self.log('Finding number of pages in {}ing leaderboard'
            ...
            for link in soup('a'):
                if link.get('title') == 'Last Page':
                    ...
                    break
            self.log('Num pages in {} leaderboard: {} '
                     .format('pitching' if pit else 'batting', num_pages))
            ids = dict()
            for page in range(1, num_pages + 1):
                ...
                res = requests.get(url, params=opts)
                if res.url.find('error') != -1:
                    self.log('Problem getting page {} from {} leaderboards'
                             .format(page, 'pitching' if pit else 'batting'))
    ...
    def wOBA_game(self, player, year=None):
        ...
        with open(csv_name, 'rt') as game_logs:
            for row in game_logs:
                ...
                d = map(int, d)
                if d[0] == year:
                    dates.insert(0, datetime.date(d[0], d[1], d[2]))
        ...
        plt.grid()
        plt.savefig('{}PerGame_wOBA.png'.format(p_tup[1].replace(' ', '')),
                    bbox_inches='tight')
    ...
    def cum_avg_wOBA(self, player):
        ...
        plt.grid()
        plt.savefig('{}CumulativeAverage_wOBA.png'
                    .format(p_tup[1].replace(' ', '')),
                    bbox_inches='tight')
